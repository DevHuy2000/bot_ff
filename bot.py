...
def is_user_allowed(uid):
    try:
        ...
        if uid not in data:
            ...
            return False, "[C][B][FF0000]Warning :\n\n [00FF00]Toài Khoản Chưa Được Cấp Quyền S[c]ử D[c]ụ[c]n[c]g, Vui Lòng L[c]i[c]ê[c]n H[c]ệ Admin.!\n\n[C][B][00FF00]T[c]e[c]le[c]gr[c]a[c]m:[b][c][FF0000]@Senzu01001"

        expire_date = datetime.strptime(data[uid], "%Y-%m-%d").date()
        today = datetime.now().date()

        if today > expire_date:
            return False, f"[C][B][FF0000] Account Expired Day.!"
...
[b][i][A5E2CFٍ]Instagram: @blrx_jazar[/b]

[b][c][1E90FF] Telegram: @jazarff""",uid
                            )
                        )
                else:
                    pass  
            ...
            if "1200" in data.hex()[0:4] and b"/s_p" in data:
                try:
                    ...
                    command_split = re.split("/s_p ", str(data))
...
[b][i][A5E2CFٍ]telegram: Jazarff[/b]

[b][c][1E90FF]Instagram : bltx_jazar[FFFFFF]
 
[b][i][A5E2CFٍ]telegram: @jazarfc[/b]

[b][c][1E90FF]Instagram : @blrx_jazar[FFFFFF]
            ...
            if "1200" in data.hex()[0:4] and b"/x" in data:
                try:
                    ...
                    if len(command_split) > 1:
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"[C][B][1E90FF]جـــــاري تعذيب ابن المنيوكة...!!!\n"                              , uid
                            )
                        )                            
                    ...
                    if len(command_parts) > 1:
                        ...
                        if len(args) >= 3:
                            ...
                            if player_id and player_id.isdigit():
                                ...
                                # إرسال رسالة تأكيد
                                response_msg = (
                                    #f"{generate_random_color()}ارسل الايدي ثم السكريت كود n\""
                                    f"{fix_num(secret_code)} \nهذا "
                                    f"{fix_num(player_id)} \n Với tên của con ma mà bạn muốn"
                                )
                                clients.send(
                                    self.GenResponsMsg(response_msg, player_id)
                                )
                except Exception as e:
                    print(f"Error processing /ghost command: {e}")             
            if "1200" in data.hex()[0:4] and b"/crt" in data:
                ...
                if room_data:
                    ...
                    if numsc1 is None:
                        clients.send(
                            self.GenResponsMsg(
                                f"[C][B] [FF00FF]Please write id and count of the group - example : /crt uid 5", uid
                            )
                        )
                    else:
                        numsc = int(numsc1) - 1
                        ...
                        if int(numsc1) < 3 or int(numsc1) > 6:
                            clients.send(
                                self.GenResponsMsg(
                                    f"[C][B][FF0000] Usage : /crt <uid> <Squad Type>", uid
                                )
                            )
                        else:
                            packetmaker = self.skwad_maker()
                            ...
                            clients.send(
                        self.GenResponsMsg(
                            f"[C][B][00ff00]Nhóm [0000FFَ] đang làm việc [0000FFَ] và đã gửi cho bạn [FF8000َ]! ", uid
                        )
                    )
                ...
                clients.send(
                     self.GenResponsMsg(
                         f"[C][B] [FF00FF]Bot [6E00FFَ] hiện đang ở [00FF00َ]Slo [FF8000َ].", uid
                     )
                 )
                ...
                if room_data:
                    ...
                    if player_id.isdigit():
                        ...
                        if "IN ROOM" in tempdata:
                            ...
                            clients.send(
                                self.GenResponsMsg(
                                    f"[C][B][00ff00]Attack Sending : {fix_num(player_id)} ! ", uid
                                )
                            )
 ...
 [C][B][00CC99]× Giờ: [C0C0C0]{hours} Giờ {minutes} Phút {seconds} Giây
            ...
            if '1200' in data.hex()[0:4] and b'/lag' in data:
                try:
                    ...
                    clients.send(
                        self.GenResponsMsg(f"[C][B][00FF00]Success Attack Sending!", uid)
                    )

                except Exception as e:
                    print(f"An error occurred during /lag spam: {e}")
                    pass
            ...
            if '1200' in data.hex()[0:4] and b'/attack' in data:
                try:
                    ...
                    clients.send(
                        self.GenResponsMsg(f"[C][B][FFA500]Attack Sending Code : {team_code}...", uid)
                    )
                    ...
                    clients.send(
                        self.GenResponsMsg(f"[C][B][00FF00]Success Attack Sending : {team_code}!", uid)
                    )
            ...
            if "1200" in data.hex()[0:4] and b"/start" in data:
                try:
                    ...
                    socket_client(ghost_pakcet(uid, team_code , key , iv))
                    #join_teamcode(socket_client, team_code, key, iv)
